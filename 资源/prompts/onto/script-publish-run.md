...
6. **本体域 code / ID** 从同实现 `快速启动_<名>.md` §1 读取；`function_id` 前缀建议与 **本体域 code** 一致；成员挂载用 `s.domain.apply_registry`（见 §4.6）。
...
**禁止**为补分类重跑完整 `*_ontology_init.py`；补发函数后**只重跑 category_mount**（及域挂载）。

### 4.6 本体域成员挂载（category_mount 之后，强制）

**平台分类 ≠ 本体域成员**。`s.categories.apply_registry` 完成后，还须将附录 B 资源挂入 **快速启动 §1 本体域**（`s.domain.apply_registry`），否则管理端本体域详情中 table/object/cube/function 仍为灰色。

在 `*_category_mount.py` **末尾**追加（域 code = §1 **本体域 code**，与 function_id 前缀一致）：

```python
DOMAIN_CODE = "profit01"  # 快速启动 §1
DOMAIN_NAME = "利润分析01"
...
DOMAIN_REGISTRY = {
    ...
    "name": DOMAIN_NAME,
    "members": {
        "table": ["dim_account", "fact_x"],
        "cube": ["ProjectProfitCube"],
        "object_type": ["Account", "ProfitAnalysis"],
        "link_type": ["budget_for_project"],
        "function": ["profit01.fn.get_summary"],
    },
}
s.domain.apply_registry(DOMAIN_REGISTRY, strict=False)
```

与 `CATEGORY_REGISTRY` 资源集一致时，可用 `_flatten_for_domain(CATEGORY_REGISTRY)` 生成 members（relation 元组暂跳过）。详见 `onto/dazi-script-sdk-reference` §5.7、`onto/script-run-troubleshooting` §6.1。

**域实体**：扩展创建或 `dazi onto domain ensure --onto-dir <实现目录>` 回写 §1 **本体域 ID**（**不**自动挂成员）。

**验收**：`dazi onto domain show --domain-id <§1 ID>` 或管理端域树 count > 0。
...
**顺序**：先 run init，再 run seed，再发布/运行函数，再 run category_mount（含 **末尾 domain.apply_registry**），最后 function run / save-test-arguments。
...
- `function-guide.md`
- `脚本运行常见错误处理.md`（API / CLI 踩坑；§1.4 分类 vs 域成员）
