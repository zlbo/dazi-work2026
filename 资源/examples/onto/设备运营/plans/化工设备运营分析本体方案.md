...
> **只读参照**：供学习化工设备运营域规划**结构**与验收口径，**不得**整篇复制为工作区 `plans/` 充数。须独立撰写并含差异说明（§1.3）。

> **任务模式**：**独立规划（greenfield）** — 按本文档 §七 全量 init/seed/11 函数。若平台 **本体域已存在**且以 gap 为主 → **待办扩展（domain-backed）**：materialize + `plans/training/gap_*.md` + `functions/gap_*_fn_*.py` 补丁，**勿**重写整域 init。见 [本体规划指南 · 任务模式](../../../../../资源/docs/onto/本体规划指南.md#任务模式与反模式)。
...
| ------- | --------------------- | ------------------- |
| equip_ops.fn.get_summary | `functions/test_arguments/equip_ops.fn.get_summary.json` | start_date=2025-01-01, end_date=2026-06-30 |
| equip_ops.fn.oee_analysis | `.../equip_ops.fn.oee_analysis.json` | 同上, group_by=plant |
| equip_ops.fn.availability_analysis | `.../equip_ops.fn.availability_analysis.json` | 同上 |
| equip_ops.fn.downtime_breakdown | `.../equip_ops.fn.downtime_breakdown.json` | reason_level=2 |
| equip_ops.fn.yoy_analysis | `.../equip_ops.fn.yoy_analysis.json` | metric=oee |
| equip_ops.fn.mom_analysis | `.../equip_ops.fn.mom_analysis.json` | metric=availability |
| equip_ops.fn.top_fault_equipment | `.../equip_ops.fn.top_fault_equipment.json` | limit=10 |
| equip_ops.fn.maintenance_compliance | `.../equip_ops.fn.maintenance_compliance.json` | 2025-01-01 ~ 2026-06-30 |
| equip_ops.fn.energy_intensity | `.../equip_ops.fn.energy_intensity.json` | group_by=plant |
| equip_ops.fn.plan_vs_actual | `.../equip_ops.fn.plan_vs_actual.json` | fiscal_year=2026, fiscal_month=6, plan_version=2026月度计划 |
| equip_ops.fn.unit_comparison | `.../equip_ops.fn.unit_comparison.json` | metric=oee, plant_id（如有） |

> 内置示例：`资源/examples/onto/设备运营/functions/test_arguments/`（与 `equip_ops_fn_*.py` 内 `TEST_ARGUMENTS` 一致）。

批量入库（dazi-work 根）：

```powershell
.\项目/DAZI_TEST/本体/ontos/设备运营/functions/save_test_arguments.ps1
```

> gap 待办补丁（domain-backed）拉取后，新函数的 JSON 落盘为 `functions/test_arguments/{function_id}.json`，见 `dazi onto training gap pull`（B3 骨架）。

---

## 七、实施计划

> **dazi-vscode v3** · 空间 `space_cate_test01` · 实现 `项目/DAZI_TEST/本体/ontos/设备运营/`  
> 单函数发布：`dazi onto script publish …/functions/equip_ops_fn_*.py --space space_cate_test01 --register-function-id equip_ops.fn.* **--mount-domain auto**`  
> bulk 入域：`equip_ops_category_mount.py` 末尾 **`s.domain.apply_registry`**（附录 B 成员键与 `equip_ops.fn.*` 前缀一致）。
...
| 22 | 11 个 `equip_ops_fn_*.py`（**publish --register-function-id --mount-domain auto**） |
| 23 | publish + run → **save-test-arguments** |
| 24 | `equip_ops_category_mount.py` 最后执行（**s.categories + s.domain**） |
...
与 `equip_ops_category_mount.py` 一致：函数 **publish** 后由脚本 **`s.categories.apply_registry` + `s.domain.apply_registry`** 挂载（`skip_missing=True` 可重跑）。
...
- [ ] 347 分类 + **本体域成员**：附录 B 完整；快速启动 §1 含 **domainId**
- [ ] 与参照示例差异说明：已填写（§1.3）
...
| 文档 | 路径 |
| ---- | ---- |
| 本体命名规范 | [本体命名规范_物理表Cube与对象.md](../../../../../资源/docs/onto/本体命名规范_物理表Cube与对象.md) |
| 本体规划指南 | [本体规划指南.md](../../../../../资源/docs/onto/本体规划指南.md) |
| 利润 / 销售示例 | [利润示例 plans](../../利润示例/plans/规划示例_利润分析本体方案.md) |
| **内置脚本示例** | `资源/examples/onto/设备运营/`（`equip_ops_fn_oee_analysis.py` 等为 gap 骨架参照） |
| 分类与域挂载 | [本体分类规划与 SDK 扩展方案](../../../../../资源/docs/onto/本体分类规划与SDK扩展方案.md) |

---

**方案版本**：v1.1（greenfield vs domain-backed、`--mount-domain`、test_arguments 路径）  
**适用**：`项目/DAZI_TEST/本体/ontos/设备运营/` 或复制后按业务改造  
**状态**：规划示例（公共模板）
