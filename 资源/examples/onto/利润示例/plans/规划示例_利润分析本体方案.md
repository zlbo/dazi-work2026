...
> **只读参照**：本文档供学习 GL 域规划**结构**与验收口径，**不得**整篇复制为本项目 `plans/` 充数。新建实现须在 `plans/` **独立撰写**，并含「与参照示例的差异说明」章节（见 [本体规划指南 · 任务模式](../../../../../资源/docs/onto/本体规划指南.md#任务模式与反模式)）。

> **任务模式**：本文档适用于 **独立规划（greenfield）** — 从零定稿 `plans/` 后按 §七 实施 init/seed/functions。若平台 **本体域已存在**且以问数 gap 为主，请走 **待办扩展（domain-backed）**：侧栏 materialize → 读 `context/domain_inventory.md` 与 `plans/training/gap_*.md` 写**最小补丁**，**勿**整篇复制本文档。
...
> setup 发布：`dazi onto script publish <item-path>/setup/profit_ontology_init.py --space <space-id> --type setup`  
> 单函数发布：`dazi onto script publish <item-path>/functions/profit_fn_*.py --space <space-id> --register-function-id profit.fn.* **--mount-domain auto**`（单函数入域；表/Cube 等 bulk 仍靠 `profit_category_mount.py` 内 **`s.domain.apply_registry`**）
...
| 16 | 7 个 `profit_fn_*.py`：**publish --register-function-id --mount-domain auto** |
...
函数在 `functions/` 中 **publish 注册**（建议 **`--mount-domain auto`**）；`profit_category_mount.py` 在 **init + seed + 全部函数 publish 之后** 统一 `s.categories.apply_registry` + **`s.domain.apply_registry`**（含 function 段，`skip_missing=True` 可重跑）。建议分类：
...
| 5 | 对象 + 分类 | ☑（§5.1、§5.4） |
| 6–10 | 同指南 | ☑ |
| 6a–6e | 平台侧栏分类 + **本体域成员** | ☑（附录 B + `profit_category_mount.py`；§1 含 domainId） |
...
**方案版本**：v2.4（补充 greenfield vs domain-backed、`--mount-domain auto` / `s.domain` 说明）  
**创建日期**：（定稿日）  
