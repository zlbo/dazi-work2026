...
| **待办扩展**（domain-backed） | 读 `context/domain_inventory.md` + `plans/training/gap_*.md`；写最小补丁 | 无 gap 理由重写整域 init |

**greenfield vs domain-backed 决策**（027 §8）：

| 情况 | 推荐路径 | 快速启动 §0 默认 |
| ---- | -------- | ---------------- |
| 从零规划新业务域 | 侧栏 **新建本体实现**（greenfield） | 独立规划 |
| 平台域已存在、以问数 gap 为主 | 侧栏 **本体域 → materialize** | 待办扩展 |
| 已有 greenfield 且 plans+init 齐全 | 原实现 **gaps pull** 即可 | 保持原模式 |
| 平台与本地脚本漂移 | `snapshotPull` 刷新 `context/` | — |
...
v3 每个本体实现绑定 **本体域**（`快速启动 §1`：`本体域 code` / **本体域 ID**）。域成员决定管理端 **本体域详情树** 是否展示本实现的表/Cube/对象/函数。

| 规划要求 | 说明 |
| -------- | ---- |
| §1 域字段 | 扩展创建或 `dazi onto domain ensure` 回写 **domainId** |
| 附录 B 资源集 | 与 **域成员** `DOMAIN_REGISTRY.members` 业务键一致（table 名、Cube 名、object code、link code、function_id） |
| 域挂载脚本 | `category_mount` **之后** `s.domain.apply_registry`（同文件末尾或独立 `*_domain_mount.py`） |
| function_id 前缀 | 建议 = **本体域 code**（如 `profit01.fn.get_summary`） |

> **勿混淆**：`s.categories.apply_registry` 只挂 **平台分类**；不写入 `ontology_domain_members`。仅跑 category_mount 时域内 kind 仍为灰色（link/relation 因前端空间级入口可能看似有内容）。

详见 [SDK 参考 §5.7](./dazi_script_sdk_reference.md)、[本体脚本编写指南 §本体域成员挂载](./本体脚本编写指南.md#本体域成员挂载sdomain强制)。
...
| **附录 C：本体域成员对照**（推荐） | `DOMAIN_REGISTRY.members` 业务键清单；与附录 B 资源集一致 |
...
6. **同脚本末尾或 `*_domain_mount.py`**：`s.domain.apply_registry(DOMAIN_REGISTRY)`（**本体域成员**；域 code 见快速启动 §1）

实施前可用 `dazi onto function list --space <space_id>` 核对平台已有函数定义；`dazi onto space get <space_id>` 核对 **`relationship_count`** 是否与规划一致。域成员验收：`dazi onto domain show --domain-id <§1 本体域 ID>`。
...
| 6f | **本体域成员** | 已 `s.domain.apply_registry`；管理端本体域（§1 ID）table/object/cube/function **count > 0** |

**命名规范扩展项**（详见 [本体命名规范 §8.4](./本体命名规范_物理表Cube与对象.md#84-自检清单在-本体规划指南-原-10-项基础上扩展)）：`dim_date` / `date_key`、表前缀、Cube 类别、对象分类、三层对照表、**平台分类 6a–6e**、**域成员 6f**。
