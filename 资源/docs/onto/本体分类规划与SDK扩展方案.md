...
> **与本体域成员区分**：本文 **平台分类**（`s.categories` → `ads_categories`）管侧栏分组；**本体域成员**（`s.domain` → `ontology_domain_members`）管管理端 **本体域详情树**。两者 **都要做**：先 `category_mount`，再 `domain.apply_registry`。见 [SDK 参考 §5.7](./dazi_script_sdk_reference.md)。
...
| 域成员 | 同脚本末尾或 `*_domain_mount.py` | `s.domain.apply_registry`（域 code = 快速启动 §1） |
...
| SDK 参考 §5.7 本体域 | `资源/docs/onto/dazi_script_sdk_reference.md` |
