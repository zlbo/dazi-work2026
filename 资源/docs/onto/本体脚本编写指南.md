...
```
init → seed → 发布全部函数 → category_mount → domain_mount（域成员）
```
...
**平台分类 ≠ 本体域成员**。仅执行 `s.categories.apply_registry` 时，管理端 **本体域详情** 中表/对象/Cube/函数等 kind 仍为 **count=0（灰色）**；须在 category_mount **之后** 将资源挂入 §1 的本体域。

| 脚本 | 时机 |
| ---- | ---- |
| `*_category_mount.py` 末尾 | 在 `apply_registry` 之后追加 `s.domain.apply_registry`（推荐） |
| 独立 `*_domain_mount.py` | 与 category_mount 同序，**最后**执行 |

**域 code**：与 `快速启动_<名>.md` §1 **本体域 code** 一致（与 `function_id` 前缀一致，如 `profit01`）。
...
附录 B 与 `CATEGORY_REGISTRY` 资源集一致时，可在 category_mount 内用 `_flatten_for_domain(CATEGORY_REGISTRY)` 生成 members（**relation 元组暂跳过**，见 [SDK 参考 §5.7](./dazi_script_sdk_reference.md)）。

**CLI（域实体，非成员）**：扩展创建时或 `dazi onto domain ensure --onto-dir <实现目录>` 幂等创建域并回写 §1 **本体域 ID**；**ensure 不挂成员**，成员仍须脚本 `apply_registry`。

**验收**：管理端本体域树各 kind count > 0；或 `dazi onto domain show --domain-id <§1 ID>`。
...
6. **本体域成员**：已 `s.domain.apply_registry`（或独立 domain_mount）？管理端域树 count > 0？

任一步不满足 → **视为未注册、未分类或未入域**，需补 publish / 补跑 `category_mount` / 补跑域挂载，不要仅 `script run` 以为函数已可用。
...
### 发布后入域（`--mount-domain`，Phase C）

| 选项 | 默认 | 说明 |
| ---- | ---- | ---- |
| `--mount-domain auto` | ✅ | 发布成功后挂载 **script** + 已注册 **function** 到 §1 本体域 |
| `--mount-domain off` | | 跳过域成员挂载 |
| `--domain-id <id>` | | 显式域 ID（优先于快速启动 §1） |

**注意**：`--mount-domain` **不替代** `category_mount` 中的 `s.domain.apply_registry`（表/Cube/对象/链接等 bulk 入域仍靠该脚本）。

```powershell
dazi onto script publish …/functions/xxx.py --space <space-id> `
  --register-function-id profit01.fn.xxx --mount-domain auto
```
...
| **本体域成员已挂载** | 管理端 **本体域**（§1 ID）下 table/object/cube/function count > 0；非仅 link/relation 可展开 |
