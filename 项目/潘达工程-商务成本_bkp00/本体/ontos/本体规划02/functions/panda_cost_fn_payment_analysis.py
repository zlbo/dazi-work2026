"""付款与未付款分析 panda_cost.fn.payment_analysis
...
发布：
  dazi onto script publish 项目/潘达工程-商务成本/本体/ontos/本体规划02/functions/panda_cost_fn_payment_analysis.py \
...
def _ontology_fn_body(p):
    ...
    for row in rows:
        ...
        data.append({
            ...
            "payable_unconfirmed": round(float(row.get("payable_unconfirmed") or 0), 2),
            "paid_amount": round(paid_amount, 2),
            "unpaid_amount": round(unpaid_amount, 2),
            "payment_rate": round(payment_rate, 4),
            "labor_payable": round(float(row.get("labor_payable") or 0), 2),
