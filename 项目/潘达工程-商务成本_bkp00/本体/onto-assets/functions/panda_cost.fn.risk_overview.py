# 本体函数脚本（从服务端拉取）
# function_id: panda_cost.fn.risk_overview
# script_id: fc0ad54d-f985-4b32-a62b-804ffdbf0d52
# space_id: space__panda_construction

"""风险预警总览 panda_cost.fn.risk_overview（V3）

参数：report_period, warning_level（可选）
返回：risk_type, risk_count, high_risk_count, avg_risk_value, warning_level, report_period

V3：warning_level 枚举 绿/黄/红

发布：
  dazi onto script publish 项目/潘达工程-商务成本/本体/ontos/本体规划03/functions/panda_cost_v3_fn_risk_overview.py \
...
def _ontology_fn_body(p):
    ...
    SELECT
        risk_type,
        count() AS risk_count,
        countIf(warning_level IN ('red', '高', '红')) AS high_risk_count,
    ...
    for row in rows:
        data.append({
            ...
            "avg_risk_value": _safe_round(row.get("avg_risk_value"), 2),
