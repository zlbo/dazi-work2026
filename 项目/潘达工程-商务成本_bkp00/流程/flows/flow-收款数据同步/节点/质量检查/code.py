# -*- coding: utf-8 -*-
# 收款同步 - 质量检查节点
# output_variable_name = validated_receipt_data

import pandas as pd

output.print("[收款同步] 开始数据质量检查")

# 获取上游数据
df = get_variable("project_receipt_data")
...
# 1. 必填字段检查
required_fields = ['receipt_id', 'project_id', 'report_period']
...
# receipt_id 检查（警告级别，不剔除数据）
if 'receipt_id' in df.columns:
    receipt_id_missing = df['receipt_id'].isnull().sum()
    if receipt_id_missing > 0:
        warnings.append(f"receipt_id 字段有 {receipt_id_missing} 个空值")
...
for _, row in df.iterrows():
    ...
    for col in df.columns:
        ...
        if hasattr(val, 'dtype'):
            ...
            if col in ['receipt_id', 'contract_id', 'contract_name', 'report_period', 
                      'project_id', 'contract_code']:
                result_row[col] = ''
