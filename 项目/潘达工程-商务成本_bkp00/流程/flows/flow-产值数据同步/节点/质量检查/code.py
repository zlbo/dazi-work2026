# -*- coding: utf-8 -*-
# 产值同步 - 质量检查节点
# output_variable_name = validated_output_data

import pandas as pd

output.print("[产值同步] 开始数据质量检查")

# 获取上游数据
df = get_variable("project_output_data")
...
# 1. 必填字段检查
required_fields = ['project_id', 'report_period']
...
# output_id 检查（警告级别，不剔除数据）
if 'output_id' in df.columns:
    output_id_missing = df['output_id'].isnull().sum()
    if output_id_missing > 0:
        warnings.append(f"output_id 字段有 {output_id_missing} 个空值")
...
for _, row in df.iterrows():
    ...
    for col in df.columns:
        ...
        if hasattr(val, 'dtype'):
            ...
            # 根据字段类型处理空值
            if col in ['output_id', 'project_id', 'project_name', 'report_period', 'company_id']:
                result_row[col] = ''
