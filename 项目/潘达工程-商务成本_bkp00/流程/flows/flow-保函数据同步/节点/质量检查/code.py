# -*- coding: utf-8 -*-
# 保函同步 - 质量检查节点
# output_variable_name = validated_guarantee_data

import pandas as pd

output.print("[保函同步] 开始数据质量检查")
df = get_variable("project_guarantee_data")
output.print(f"输入数据 shape={df.shape}")

errors = []
warnings = []

required_fields = ['guarantee_id', 'project_id', 'report_period']
...
if 'guarantee_id' in df.columns:
    guarantee_id_missing = df['guarantee_id'].isnull().sum()
    if guarantee_id_missing > 0:
        warnings.append(f"guarantee_id 字段有 {guarantee_id_missing} 个空值")
...
for _, row in df.iterrows():
    ...
    for col in df.columns:
        ...
        if hasattr(val, 'dtype'):
            ...
            if col in ['guarantee_id', 'project_id', 'contract_id', 'guarantee_type', 'guarantee_status', 'report_period']:
                result_row[col] = ''
