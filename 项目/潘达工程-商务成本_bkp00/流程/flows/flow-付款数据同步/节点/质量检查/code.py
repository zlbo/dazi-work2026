# -*- coding: utf-8 -*-
# 付款同步 - 质量检查节点
# output_variable_name = validated_payment_data

import pandas as pd

output.print("[付款同步] 开始数据质量检查")

# 获取上游数据
df = get_variable("project_payment_data")
...
# 1. 必填字段检查
required_fields = ['payment_id', 'project_id', 'report_period']
...
# payment_id 检查（警告级别，不剔除数据）
if 'payment_id' in df.columns:
    payment_id_missing = df['payment_id'].isnull().sum()
    if payment_id_missing > 0:
        warnings.append(f"payment_id 字段有 {payment_id_missing} 个空值")
...
for _, row in df.iterrows():
    ...
    for col in df.columns:
        ...
        if hasattr(val, 'dtype'):
            ...
            # 根据字段类型处理空值
            if col in ['payment_id', 'contract_id', 'contract_name', 'settlement_status', 
                      'supplier_id', 'report_period', 'approval_status', 
                      'project_id', 'supplier_name', 'contract_code']:
                result_row[col] = ''
