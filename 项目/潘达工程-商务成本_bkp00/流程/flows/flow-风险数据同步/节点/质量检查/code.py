# -*- coding: utf-8 -*-
# 风险同步 - 质量检查节点
# output_variable_name = validated_risk_data

import pandas as pd

output.print("[风险同步] 开始数据质量检查")

# 获取上游数据
df = get_variable("project_risk_data")
...
# 1. 必填字段检查
required_fields = ['risk_id', 'project_id', 'report_period']
...
for _, row in df.iterrows():
    ...
    for col in df.columns:
        ...
        if hasattr(val, 'dtype'):
            ...
            # 根据字段类型处理空值
            if col in ['risk_id', 'project_id', 'report_period', 'risk_type', 'risk_code', 'risk_name', 'warning_level', 'risk_description', 'overall_warning_level']:
                result_row[col] = ''
            elif col in ['date_key', 'risk_value']:
                result_row[col] = 0
            else:
                result_row[col] = val
        else:
