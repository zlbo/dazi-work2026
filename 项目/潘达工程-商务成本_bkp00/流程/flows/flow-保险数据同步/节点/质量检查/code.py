# -*- coding: utf-8 -*-
# 保险同步 - 质量检查节点
# output_variable_name = validated_insurance_data

import pandas as pd

output.print("[保险同步] 开始数据质量检查")
df = get_variable("project_insurance_data")
output.print(f"输入数据 shape={df.shape}")

errors = []
warnings = []

required_fields = ['insurance_id', 'project_id', 'report_period']
...
if 'insurance_id' in df.columns:
    insurance_id_missing = df['insurance_id'].isnull().sum()
    if insurance_id_missing > 0:
        warnings.append(f"insurance_id 字段有 {insurance_id_missing} 个空值")
...
for _, row in df.iterrows():
    ...
    for col in df.columns:
        ...
        if hasattr(val, 'dtype'):
            ...
            if col in ['insurance_id', 'project_id', 'contract_id', 'insurance_type', 'insurance_status', 'insurance_company', 'report_period']:
                result_row[col] = ''
