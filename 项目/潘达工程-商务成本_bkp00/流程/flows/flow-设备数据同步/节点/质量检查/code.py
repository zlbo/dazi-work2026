# -*- coding: utf-8 -*-
# 设备同步 - 质量检查节点
# output_variable_name = validated_equipment_data

import pandas as pd

output.print("[设备同步] 开始数据质量检查")
df = get_variable("project_equipment_data")
output.print(f"输入数据 shape={df.shape}")

errors = []
warnings = []

required_fields = ['equipment_id', 'project_id', 'report_period']
...
if 'equipment_id' in df.columns:
    equipment_id_missing = df['equipment_id'].isnull().sum()
    if equipment_id_missing > 0:
        warnings.append(f"equipment_id 字段有 {equipment_id_missing} 个空值")
...
for _, row in df.iterrows():
    ...
    for col in df.columns:
        ...
        if hasattr(val, 'dtype'):
            ...
            if col in ['equipment_id', 'project_id', 'contract_id', 'equipment_code', 'equipment_name', 'equipment_type', 'equipment_status', 'depreciation_method', 'report_period']:
                result_row[col] = ''
