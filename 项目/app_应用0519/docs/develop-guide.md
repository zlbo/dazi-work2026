...
**原则**：模板在 `templates/`，应用在 `apps/`；在 monorepo **根**执行 `pnpm run dazi-app`；在**应用目录**用 `pnpm -C ../.. run dazi-app -- <子命令> --cwd .`。
...
```powershell
pnpm run dazi-app -- init production-command-center `
  --space space__0519 `
  --dir apps/my-pcc `
  --name "我的生产指挥中心" `
...
```powershell
pnpm run dazi-app -- context refresh --cwd apps/my-pcc
...
```powershell
pnpm run dazi-app -- manifest diff-template production-command-center --cwd apps/my-pcc
pnpm run dazi-app -- manifest merge-template production-command-center --cwd apps/my-pcc --profile sql-script --write
```
