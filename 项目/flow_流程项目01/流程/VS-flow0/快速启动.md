...
| flowId | `103` |
| 本地目录 | `流程/VS-flow0/` |
| 节点数 | 7（代码节点 4 + database-sink 1） |
| 上次拉取 | 2026-05-30T14:38:49.212Z |

## 流程拓扑

```text
开始 → Excel 多表解析 → 数据质量检查 → 质检是否通过 ──True──→ SQL 表间关联 → 写入销售临时表 → 结束
                                              └──False──→ 结束
```

**入库**：`销售明细宽表`（96 行）→ `clickhouse__space_xsh_01`.`temp_sales001`（销售表临时数据001）

**本流程总结与最佳实践** → [flow-dev-best-practices.md](../../../../资源/docs/flow/flow-dev-best-practices.md)（含节点选型、变量约定、检查表，供其他流程参考）
...
```powershell
cd "项目\flow_流程项目01\流程\VS-flow0"

# 状态 / 拉取 / 提交
.\scripts\dazi.ps1 flow project status --dir .
.\scripts\dazi.ps1 flow project pull --flow 103 --dir .
...
| **流程开发最佳实践（VS-flow0 案例）** | [flow-dev-best-practices.md](../../../../资源/docs/flow/flow-dev-best-practices.md) |
