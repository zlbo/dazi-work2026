...
功能：指定指标同比变化分析
参数：start_date, end_date, metric（gross_profit/revenue/total_cost/profit_margin）, plant_id（可选）

放置：项目/DAZI_TEST/本体/ontos/利润成本/functions/profit_fn_yoy_analysis.py
发布：dazi onto script publish 项目/DAZI_TEST/本体/ontos/利润成本/functions/profit_fn_yoy_analysis.py --space space_cate_test01 --register-function-id profit.fn.yoy_analysis --register-platform-category 趋势分析
"""
...
def _ontology_fn_body(p):
    ...
    prev_sql = f"""
        SELECT
            ...
            {expr} as metric_value
        FROM fact_gl_journal_entry
        WHERE posting_date >= '{prev_start}' AND posting_date <= '{prev_end}'
            {plant_cond}
        GROUP BY toYear(posting_date), toMonth(posting_date)
        ORDER BY year, month
    """
    ...
    prev_rows = p.sql.query(prev_sql)

    prev_dict = {r["month"]: r["metric_value"] for r in prev_rows}

    data = []
    for row in rows:
        current = row.get("metric_value", 0) or 0
        prev = prev_dict.get(row["month"], 0) or 0
        yoy_change = current - prev
        yoy_pct = yoy_change / abs(prev) if prev != 0 else 0
    ...
    return p.function_result(
        columns=["year", "month", "current_value", "prev_value", "yoy_change", "yoy_pct"],
        data=data,
        row_count=len(data),
    )
