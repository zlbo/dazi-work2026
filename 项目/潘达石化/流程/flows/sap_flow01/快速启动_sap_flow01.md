...
| flowId | `105` |
| 本地目录 | `流程/sap_flow01/` |
| 画布节点（flow.json） | 4，其中代码节点 2 |
| meta 已索引代码 | 2 |
| 上次拉取 | 2026-06-04T22:40:29.796Z |
...
```powershell
cd "项目\flow_<项目名>\流程\flows\sap_flow01"

# 状态 / 拉取 / 提交
dazi flow project status --dir .
dazi flow project pull --flow 105 --dir .
...
2. 运行节点：`dazi flow run node-exec --node <node_uuid> --dir .`
3. 提交代码：`dazi flow node push --node <node_uuid> --dir .`
...
4. 必要时重新拉取校对：`dazi flow project pull --flow 105 --dir .`
