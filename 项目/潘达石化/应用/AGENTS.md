...
```powershell
# 在应用项目根（含 sdk/、templates/、apps/）
cd .    # 或 cd 项目/app_<名称>

# 1. 脚手架（亦可用扩展「在已有项目下新建组件」）
pnpm run dazi-app -- init <template_id> --space <spaceId> --dir apps/<app_id>
...
# 5. 发布
pnpm -C ../.. run dazi-app -- upload --cwd . --space <spaceId> --activate
```
...
```powershell
pnpm run dazi-app -- help [quickstart|manifest|data-source|upload|doctor]
pnpm run dazi-app -- doctor [--cwd apps/<app_id>]
pnpm run dazi-app -- manifest validate --scan-src
pnpm run dazi-app -- context refresh
```
