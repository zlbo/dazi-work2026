...
在工作区根目录终端执行（需已 `dazi auth login`，推荐用 `dazi`）：

```powershell
# 1a. 发布初始化脚本
dazi onto script publish 项目/onto_本体测试05/脚本/setup/inventory_ontology_init.py --space space__0519

# 1b. 执行初始化（建表 + 注册表 + Cube + 6 种对象类型 + 10 条链接）— 必做
dazi onto script run --script-id <init 的 scriptId> --space space__0519

# 2a. 发布灌数脚本
dazi onto script publish 项目/onto_本体测试05/脚本/setup/inventory_seed_data.py --space space__0519

# 2b. 执行灌数 — 必做
dazi onto script run --script-id <seed 的 scriptId> --space space__0519
...
dazi onto function run inventory.fn.get_summary --space space__0519 --params '{}'
```
