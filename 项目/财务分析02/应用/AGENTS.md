...
```powershell
# 推荐：在 dazi-work 根（.\scripts\dazi.ps1 自动设置 bundled CLI）
cd D:\path\to\dazi-work

# 1. 脚手架（亦可用扩展「新建项目 / 新建应用组件」）
.\scripts\dazi.ps1 app init <template_id> --space <spaceId> --dir 项目/app_<名>/apps/<app_id>

# 2. 改 manifest.data_sources（VS Code 绑定面板或手改 manifest.json）
.\scripts\dazi.ps1 app manifest validate --cwd 项目/app_<名>/apps/<app_id> --scan-src
...
# 4. 自检（回到 dazi-work 根）
cd D:\path\to\dazi-work
.\scripts\dazi.ps1 app doctor --cwd 项目/app_<名>/apps/<app_id>

# 5. 发布
.\scripts\dazi.ps1 app upload --cwd 项目/app_<名>/apps/<app_id> --space <spaceId> --activate
```
...
.\scripts\dazi.ps1 app context refresh --cwd 项目/app_<名>/apps/<app_id>
```
