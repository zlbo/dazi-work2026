...
**流程**：科研项目参与人员_数据清洗与统计 · 平台 flowId `120`
...
|------|-----|
| flowId | `120` |
| 本地目录 | `流程/科研项目参与人员_数据清洗与统计/` |
| 画布节点（flow.json） | 7，其中代码节点 5 |
| meta 已索引代码 | 5 |
| 上次拉取 | 2026-06-04T03:59:47.889Z |
...
| 画布代码节点（flow.json） | 5 |
| meta 已登记节点 | 7 |
| meta 已索引代码（dir+codeFile） | 5 |

⚠️ **目录不一致**（会导致设计器找不到代码、或 `node push` 跳过）：

- 未索引目录：`节点/科研项目人员统计分析`
- 存在未索引的节点目录：节点/科研项目人员统计分析
...
```powershell
cd "项目\flow_DEMO_102_Flows\流程\科研项目参与人员_数据清洗与统计"

# 状态 / 拉取 / 提交
dazi flow project status --dir .
dazi flow project pull --flow 120 --dir .
...
4. 必要时重新拉取校对：`dazi flow project pull --flow 120 --dir .`
