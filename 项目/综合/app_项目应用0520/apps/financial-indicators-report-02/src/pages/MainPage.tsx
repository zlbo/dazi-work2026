...
import { financialReportLayout, normalizeReportLayout } from "../fixtures/reportLayout";
...
export function MainPage() {
  ...
  const ds = useReportDataset({
    visibleFields,
    wideTable: layout.wideTable,
  });

  const handleVisibleFieldsChange = useCallback((fields: string[]) => {
    setVisibleFields((prev) => {
      const a = prev.filter(Boolean).join("\0");
      const b = fields.filter(Boolean).join("\0");
