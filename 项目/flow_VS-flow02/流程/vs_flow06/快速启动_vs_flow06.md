...
```powershell
cd "项目\flow_VS-flow02\流程\vs_flow06"

# 状态 / 拉取 / 提交
.\\scripts\\dazi.ps1 flow project status --dir .
.\\scripts\\dazi.ps1 flow project pull --flow 117 --dir .
...
| type                       | 名称           |   代码   | 用途                                    | 变量                                                                          |
| -------------------------- | -------------- | :------: | --------------------------------------- | ----------------------------------------------------------------------------- |
| `file-source`              | 文件输入       |    —     | 从已上传/托管文件读取为表变量           | 出:output_variable_name                                                       |
| `excel-import`             | Excel 导入     |    —     | 单 Sheet Excel 导入为表变量（简单场景） | 出:output_variable_name                                                       |
| `excel-python`             | Excel 开发     | code.py  | 多 Sheet / 复杂表头：Python 解析 Excel  | 出:output_variable_name（result_df）                                          |
...
- 单 Sheet Excel → `excel-import`；多 Sheet / 复杂表头 → `excel-python`
- 读外部库 → `database-source`（`connectionId`）；读数据空间 → `dataspace-source`（`spaceId`）
...
4. 必要时重新拉取校对：`.\\scripts\\dazi.ps1 flow project pull --flow 117 --dir .`
