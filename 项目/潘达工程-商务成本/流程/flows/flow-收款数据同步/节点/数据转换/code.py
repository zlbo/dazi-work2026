# -*- coding: utf-8 -*-
# 收款同步 - 数据转换节点
# output_variable_name = project_receipt_data

import pandas as pd
import numpy as np

output.print("[收款同步] 开始数据转换")

# 获取上游数据
df = get_variable("receipt_raw_data")
output.print(f"输入数据 shape={df.shape}")
output.print(f"输入列: {list(df.columns)}")

# 定义目标表字段
target_fields = [
    'receipt_id', 'date_key', 'contract_id', 'contract_name', 
    'contract_amount', 'receipt_amount', 'receipt_ratio',
    'report_period', 'project_id', 'created_at', 'contract_code'
]
...
for _, row in df.iterrows():
    ...
    # 确保所有值都是 Python 原生类型
    receipt_id = str(row['id']) if pd.notnull(row['id']) else ''
    ...
    receipt_amount = float(row['receipt_amount_acc']) if pd.notnull(row['receipt_amount_acc']) else 0.0
    receipt_ratio = float(row['receipt_ratio_acc']) if pd.notnull(row['receipt_ratio_acc']) else 0.0
