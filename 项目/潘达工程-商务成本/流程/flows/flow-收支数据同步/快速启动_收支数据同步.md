...
|------|-----|
| flowId | `136` |
| 本地目录 | `项目/潘达工程-商务成本/流程/flows/收支数据同步/` |
| 画布节点（flow.json） | 6，其中代码节点 3 |
| meta 已索引代码 | 3 |
| 上次拉取 | 2026-06-10T12:33:33.301Z |
...
⚠️ **目录不一致**（会导致设计器找不到代码、或 `node push` 跳过）：

- meta 多余 uuid：2 个（flow.json 中已不存在）
...
详见 [流程本地文件规范](../../../../资源/docs/flow/local-files-spec.md)。
...
```powershell
dazi flow project status --dir "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步"
dazi flow project pull --flow 136 --dir "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步"
dazi flow project push --dir "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步" --canvas
dazi flow node push --node <node_uuid> --dir "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步"
dazi flow run node-exec --node <node_uuid> --dir "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步"
dazi flow variable pull --name <output_variable_name> --dir "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步"
dazi flow run flow-exec --dir "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步" --type debug
dazi flow variable sync --dir "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步"
```
...
```powershell
cd "d:\GitHub\dazi-work\项目\潘达工程-商务成本\流程\flows\flow-收支数据同步"
dazi flow project status --dir .
dazi flow project pull --flow 136 --dir .
```
...
dazi flow project pull --flow 136 --dir "<FLOW_DIR>"
...
4. 必要时重新拉取校对：`dazi flow project pull --flow 136 --dir .`
