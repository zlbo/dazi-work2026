# -*- coding: utf-8 -*-
# 收支同步 - 质量检查节点
# output_variable_name = validated_balance_data

import pandas as pd

output.print("[收支同步] 开始数据质量检查")

# 获取上游数据
df = get_variable("project_balance_data")
...
# 1. 必填字段检查
required_fields = ['balance_id', 'project_id', 'report_period']
...
for _, row in df.iterrows():
    ...
    for col in df.columns:
        ...
        if hasattr(val, 'dtype'):
            ...
            # 根据字段类型处理空值
            if col in ['balance_id', 'project_id', 'report_period', 'subject_code', 'subject_name']:
                result_row[col] = ''
