# 本体函数脚本（从服务端拉取）
# function_id: panda_cost.fn.mom_analysis
# script_id: 37becd59-e5e0-4ddd-85d6-7774cbccc890
# space_id: space__panda_construction

"""环比分析 panda_cost.fn.mom_analysis（V3）
...
返回：current_value, previous_month_value, growth_rate, growth_amount, metric, report_period

V3：collection_rate 优先 fact_receipt 聚合

发布：
  dazi onto script publish 项目/潘达工程-商务成本/本体/ontos/本体规划03/functions/panda_cost_v3_fn_mom_analysis.py \
    --space space__panda_construction --register-function-id panda_cost.fn.mom_analysis
"""

TEST_ARGUMENTS = {
    "v": 1,
    "arguments": {"report_period": "2025-06", "metric": "output_total"},
...
def _prev_month_period(report_period):
...
def _ontology_fn_body(p):
    ...
    metric = params.get("metric", "output_total")
    project_id = params.get("project_id") or None

    prev_period = _prev_month_period(report_period)
    ...
    data = [{
        ...
        "previous_month_value": _safe_round(previous_value, 4),
    ...
    return p.function_result(
        columns=["current_value", "previous_month_value", "growth_rate",
