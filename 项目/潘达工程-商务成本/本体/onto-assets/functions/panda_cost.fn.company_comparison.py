# 本体函数脚本（从服务端拉取）
# function_id: panda_cost.fn.company_comparison
# script_id: 07a9059d-0bef-463e-8e2c-32bb59241ffc
# space_id: space__panda_construction

"""分公司对标 panda_cost.fn.company_comparison（V3）

参数：report_period, metric
返回：rank, company_id, company_name, metric, metric_value, report_period

V3：通过 dim_company 关联对标

发布：
  dazi onto script publish 项目/潘达工程-商务成本/本体/ontos/本体规划03/functions/panda_cost_v3_fn_company_comparison.py \
...
def _ontology_fn_body(p):
    ...
    if metric == "health_score":
        ...
        SELECT
            ...
            avg(i.indicator_value) AS metric_value
        ...
        WHERE i.report_period = '{report_period}' AND i.indicator_code = '{metric}'
        ...
        scored = [{
            ...
            "metric_value": _safe_round(r.get("metric_value"), 4),
        } for r in ind_rows]
    else:
        sql = f"""
        SELECT
